...
function ViewTable() {
    ...
    const fetchUniqueTableNumbers = async () => {
        try {
            const response = await fetch(`${API_ENDPOINT}/api/customer_orders_table/`);
    ...
    const getOrderTime = (order_time) => {
        return order_time ? new Date(order_time).toLocaleTimeString([], { timeStyle: "short" }) : null;
    };

    return (
        <div className='viewTable-container'>
            <div className='viewTable-title'> <span>LUMA</span> orders</div>
            <div className='table-container'>
                {uniqueTableNumbers.map((item) => (
                    item.state === "Empty" ? (
                        ...
                        <Link to="/viewOrder" key={item.id} onClick={() => handleSelectedTable(item.table)}>
                            <EmptyTableComponent
